...
eligible_models <- c(as.character(eligible_models$Model), "CFA-PyRenew_E")
eligible_models <- eligible_models[eligible_models != "CFA-PyRenew_HE"]
...
all_forecasts_data <- forecasttools::pivot_hubverse_quantiles_wider(
  ...
  dplyr::mutate(
    ...
    dplyr::across(dplyr::starts_with("quantile_"), ~ round(.x, 4), .names = "{.col}_rounded")
  ) |>
  dplyr::left_join(
...
map_data <-forecasttools::pivot_hubverse_quantiles_wider(
  ...
  # and rounded values
  dplyr::mutate(
    quantile_0.025_per100k = NA,
    quantile_0.5_per100k = NA,
    quantile_0.975_per100k = NA,
    quantile_0.025_per100k_rounded = NA,
    quantile_0.5_per100k_rounded = NA,
    quantile_0.975_per100k_rounded = NA,
    quantile_0.025_rounded = round(quantile_0.025, 4),
    quantile_0.5_rounded = round(quantile_0.5, 4),
    quantile_0.975_rounded = round(quantile_0.975, 4),
...
flu_data_path <- paste0("C:/Users/", Sys.info()["user"], "/Desktop/GitHub/FluSight-forecast-hub")
flu_data <- read.csv(paste0(flu_data_path,"/target-data/target-ed-visits-prop.csv"))
