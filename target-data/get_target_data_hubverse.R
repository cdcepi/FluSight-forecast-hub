...
#' @return A dataframe
get_base_target_data <- function(target_data_path, as_of = NULL, target) {
  ...
  base_target_data <- readr::read_csv(file = file_name) %>% mutate(target=target)
...
get_base_target_data <- function(target_data_path, as_of = NULL) {
  ...
  # Function to read a file based on the get_latest flag
  read_target_data <- function(file_name) {
    cli::cli_inform(paste0("Reading target data from ", file_name, "..."))
    data <- readr::read_csv(file = file_name)
    return(data)
  }
  
  # Read hospital admissions data
  if (get_latest) {
    hospital_file_name <- file.path(target_data_path, "target-hospital-admissions.csv")
  } else {
    hospital_file_name <- file.path(
      here::here(),
      "auxiliary-data",
      "target-data-archive",
      paste0("target-hospital-admissions_", as_of, ".csv"))
  ...
  base_hospital_data <- read_target_data(hospital_file_name)
  latest_date_hospital <- max(base_hospital_data$date, na.rm = TRUE)
  
  if (!get_latest && as.Date(latest_date_hospital) != as.Date(as_of)) {
    cli::cli_alert_danger(
      paste0("Latest date in the hospital admissions data (", latest_date_hospital, ") does not match the 'as_of' date (", as_of, ").")
    )
    stop()
  }
  
  base_hospital_data <- cbind(
    base_hospital_data,
    data.frame(as_of = as.Date(latest_date_hospital))
  ) %>% mutate(target="wk inc flu hosp")
  
  # Read emergency visits data
  if (get_latest) {
    visits_file_name <- file.path(target_data_path, "target-ed-visits-prop.csv")
  } else {
    visits_file_name <- file.path(
      here::here(),
      "auxiliary-data",
      "target-data-archive",
      paste0("target-ed-visits-prop_", as_of, ".csv"))
  }
  
  base_visits_data <- read_target_data(visits_file_name)
  latest_date_visits <- max(base_visits_data$date, na.rm = TRUE)
  
  if (!get_latest && as.Date(latest_date_visits) != as.Date(as_of)) {
    cli::cli_alert_danger(
      paste0("Latest date in the emergency visits data (", latest_date_visits, ") does not match the 'as_of' date (", as_of, ").")
    )
    stop()
  }
  
  base_visits_data <- cbind(
    base_visits_data,
    data.frame(as_of = as.Date(latest_date_visits))
  ) %>% mutate(target="wk inc flu prop ed visits")
  
  # Add a 'rate' column to the visits data with NA values
  base_visits_data$weekly_rate <- NA
  # Bind both datasets together
  combined_data <- rbind(base_hospital_data, base_visits_data)
  
  base_target_data <- combined_data
  base_target_data
}
...
create_time_series_target_data <- function(weekly_data, location_data) {
  ...
  location_data <- location_data %>% dplyr::select(-population)
  time_series_wk_inc <- weekly_data |>
    dplyr::select(-location) |>
    dplyr::inner_join(location_data, by = c("location_name")) |>
    dplyr::select(date, location, location_name, value, weekly_rate, as_of, target) |> 
...
create_oracle_output_target_data <- function(time_series_target, oracle_include_after) {
  ...
  oracle_output_ed_pct <- create_oracle_output_ed_pct(time_series_target)

  oracle_output <- dplyr::bind_rows(oracle_output_wk_inc, oracle_output_rate_change, oracle_output_ed_pct)
...
create_oracle_output_wk_inc <- function(time_series_target) {
  ...
  oracle_output_wk_inc <- time_series_target %>% filter(target =="wk inc flu hosp") %>% 
    dplyr::select(target, target_end_date, location, observation, as_of)
...
#' @param time_series_target Dataframe of Hubverse-formatted time series target data.
#' @return A dataframe
create_oracle_output_ed_pct <- function(time_series_target) {
  oracle_output_ed_pct <- time_series_target %>% filter(target =="wk inc flu prop ed visits") %>% 
    dplyr::select(target, target_end_date, location, observation, as_of)

  colnames(oracle_output_ed_pct) <- c(
    "target", "target_end_date", "location", "oracle_value", "as_of")
  oracle_output_ed_pct <- oracle_output_ed_pct |>
    dplyr::cross_join(
      # add a row for each horizon defined in the modeling task
      # (except horizon -1, which is not used for scoring/viz)
      data.frame(horizon = 0:3)
...
create_target_data <- function(as_of = NULL, oracle_include_after = "2023-09-22", target_data_path) {
  ...
  location_data <- get_location_data()
  ##weekly_data_hosp <- get_base_target_data(target_data_path = "target-hospital-admissions.csv", as_of = NULL,target="wk inc flu hosp")
  ##weekly_data_ed <- get_base_target_data(target_data_path = "target-ed-visits-prop.csv", as_of = NULL, target="wk inc flu prop ed visits")
  weekly_data_all <- get_base_target_data(target_data_path = target_data_path, as_of = as_of)
  as_of <- weekly_data_all$as_of[1]
  ##weekly_data_all <- bind_rows(weekly_data_hosp, weekly_data_ed)
...
##############
  check_for_duplicates <- function(your_data) {
    # Count the total number of rows
    total_rows <- nrow(your_data)
    
    # Get the number of unique rows
    unique_rows <- your_data %>%
      distinct() %>%
      nrow()
    ...
    if (total_rows > unique_rows) {
      ...
      # Optionally, you can also return the duplicated rows
      duplicated_rows <- your_data %>%
  ...
  dup <- check_for_duplicates(oracle_output_target)
  ###################
