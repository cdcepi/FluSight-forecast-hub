...
If all is well, all checks should either be prefixed with a `b` indicating success or `b9` indicating a check was skipped, e.g.:
```
b FluSight-forecast-hub: All hub config files are valid.
b 2023-10-07-UMass-trends_ensemble.csv: File exists at path model-output/UMass-trends_ensemble/2023-10-07-UMass-trends_ensemble.csv.
b 2023-10-07-UMass-trends_ensemble.csv: File name "2022-10-22-team1-goodmodel.csv" is valid.
b 2023-10-07-UMass-trends_ensemble.csv: File directory name matches `model_id` metadata in file name.
b 2023-10-07-UMass-trends_ensemble.csv: `round_id` is valid.
b 2023-10-07-UMass-trends_ensemble.csv: File is accepted hub format.
...
```

If there are any failed checks or execution errors, the check's output will be prefixed with a `b` or `!` and include a message describing the problem.
...
```r
b All validation checks have been successful.
```
